...
def answer(question: str, docs):
    """Stream the model's answer from pre-retrieved docs, with friendly error
    handling."""
...
def render_streaming_answer(question: str, mode: str, cards) -> str:
    ...
    answer itself carries inline [Source …] citations from the prompt."""
    with st.spinner("Searching the card documents…"):
        docs = retrieve_docs(question, mode, cards)
    gen = answer(question, docs)
...
# Fail clearly (not with a stack trace) if the key was never provided.
if not os.environ.get("GOOGLE_API_KEY"):
    st.error(
        "🔑 GOOGLE_API_KEY is not configured. Add it under the app's "
